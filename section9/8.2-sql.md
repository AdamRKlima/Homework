...
```SQL
SELECT Accel, rtrim( Accel, ' sec') as edtstr
FROM evCars
...
UPDATE evCars SET accel = rtrim( Accel, ' sec')
```
<br>
...
```SQL
SELECT TopSpeed, rtrim( TopSpeed, ' km/h') as edtstr
FROM evCars
...
UPDATE evCars SET TopSpeed = rtrim( TopSpeed, ' km/h')
...
UPDATE evCars SET TopSpeed = round(TopSpeed*0.621371,1)
...
- Choose the appropriate string function from the string function list. There may be more then one way to accomplish this task. 
```SQL
SELECT "Range", rtrim( "Range", ' km') as edtstr
FROM evCars
```
...
- Write the update statement to set `Range` to equal the return value of the string function you chose in 3.2
 ```SQL
UPDATE evCars SET "Range" = rtrim( "Range", ' km')
```
<br>
...
UPDATE evCars SET "Range" = round("Range"*0.621371,1)
...
```SQL
SELECT Efficiency, rtrim(Efficiency, ' Wh/km') as newstr, FastCharge, rtrim(FastCharge, ' km/h') as newfsch FROM evCars
```
...
```SQL
UPDATE evCars set Efficiency = rtrim(Efficiency, ' Wh/km'), FastCharge = rtrim(FastCharge, ' km/h') 
``` 
<br>
...
UPDATE evCars SET FastCharge = round(FastCharge*0.621371,1)
...
```SQL
SELECT RapidCharge, replace(RapidCharge, 'Rapid charging possible', 'True'), replace(RapidCharge, 'Rapid charging not possible', 'False')
FROM evCars
